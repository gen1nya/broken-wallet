...
The UI now previews a native-segwit Bitcoin account derived from a generated mnemonic (BIP84 `m/84'/0'/0'`). It shows the account `zpub` (converted from the raw xpub to keep the scope on p2wpkh), a handful of receive/change addresses, and can query UTXOs for that account using NowNodes' Blockbook API.
...
UTXO lookups use the publicly reachable Blockbook endpoint at `https://btcbook.nownodes.io/api/v2`. During `npm run dev`, calls are proxied through Vite to avoid browser CORS limits. You can optionally provide an `api-key` header (exposed in the UI) if you have a NowNodes key to avoid rate limits. The lookup uses the derived account `zpub` and displays UTXO metadata plus the derivation path and address when available.
